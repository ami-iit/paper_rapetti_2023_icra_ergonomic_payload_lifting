...
const std::string ModuleName = "MultiRobotVisualizer";
...
int main(int argc, char* argv[])
{
    ...
    rf.setDefaultConfigFile("MultiRobotVisualizer.ini");
    ...
    for ( auto& modelName : modelNameList)
    {
        ...
        if (modelConfigurationGroup.check("modelColor"))
        {
            ...
            {
                if ( !(modelConfigurationGroup.find("modelColor").asList()->get(idx).isFloat64()) )
                {
                    yError() << LogPrefix << "'modelColor' entry [ " << idx << " ] is not valid.";
                    return EXIT_FAILURE;
                }
                modelColorVector.setVal(idx, modelConfigurationGroup.find("modelColor").asList()->get(idx).asFloat64());
            }
        ...
        {
            if ( !(modelConfigurationGroup.check("forceScalingFactor") && modelConfigurationGroup.find("forceScalingFactor").isFloat64()) )
            {
            ...
            {
                modelConfigurationMap[modelName].forceScalingFactor = modelConfigurationGroup.find("forceScalingFactor").asFloat64();
            }
        }
    ...
    for (auto& modelName : modelNameList)
    {
        ...
        modelConfigurationMap[modelName].basePosePort.open("/MultiRobotVisualizer/" + modelName + "/basePose:i");
        while (modelConfigurationMap[modelName].basePosePort.isClosed())
        {
            yInfo() << LogPrefix << "failed to open the port " << modelConfigurationMap[modelName].basePosePort.getName();
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
            if (isClosing) {
                return EXIT_FAILURE;
            }
        }
        while (!yarp.connect(modelConfigurationMap[modelName].basePosePortName, modelConfigurationMap[modelName].basePosePort.getName()))
        {
            yInfo() << LogPrefix << "failed to connect to the port"  << modelConfigurationMap[modelName].basePosePortName;
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
            if (isClosing) {
                return EXIT_FAILURE;
            }
        ...
        {
            yInfo() << LogPrefix << "no data coming from the port " << modelConfigurationMap[modelName].basePosePortName;
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
            if (isClosing) {
                return EXIT_FAILURE;
            }
        ...
        if ( !modelConfigurationMap[modelName].jointList.empty() )
        {
            ...
            modelConfigurationMap[modelName].jointPositionPort.open("/MultiRobotVisualizer/" + modelName  + "/jointPosition:i");
            while (modelConfigurationMap[modelName].jointPositionPort.isClosed())
            {
                yInfo() << LogPrefix << "failed to open the port" << modelConfigurationMap[modelName].jointPositionPort.getName();
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
                if (isClosing) {
                    return EXIT_FAILURE;
                }
            }
            while (!yarp.connect(modelConfigurationMap[modelName].jointPositionPortName, modelConfigurationMap[modelName].jointPositionPort.getName()))
            {
                yInfo() << LogPrefix << "failed to connect to the port " << modelConfigurationMap[modelName].jointPositionPortName;
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
                if (isClosing) {
                    return EXIT_FAILURE;
                }
            ...
            {
                yInfo() << LogPrefix << "no data coming from the port " << modelConfigurationMap[modelName].jointPositionPortName;
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
                if (isClosing) {
                    return EXIT_FAILURE;
                }
        ...
        if ( modelConfigurationMap[modelName].visualizeWrenches )
        {
            ...
            modelConfigurationMap[modelName].wrenchPort.open("/MultiRobotVisualizer/" + modelName + "/wrench:i");
            while (modelConfigurationMap[modelName].wrenchPort.isClosed())
            {
                yInfo() << LogPrefix << "failed to open the port" << modelConfigurationMap[modelName].wrenchPort.getName();
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
                if (isClosing) {
                    return EXIT_FAILURE;
                }
            }
            while (!yarp.connect(modelConfigurationMap[modelName].wrenchPortName, modelConfigurationMap[modelName].wrenchPort.getName()))
            {
                yInfo() << LogPrefix << "failed to connect to the port " << modelConfigurationMap[modelName].wrenchPortName;
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
                if (isClosing) {
                    return EXIT_FAILURE;
                }
            ...
            {
                yInfo() << LogPrefix << "no data coming from the port " << modelConfigurationMap[modelName].wrenchPortName;
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
                if (isClosing) {
                    return EXIT_FAILURE;
                }
    ...
    while(viz.run() && !isClosing)
    {
        ...
        for (auto& modelName : modelNameList)
        {
            // check if the used port exists, if not try to connect
            if ( !yarp.isConnected(modelConfigurationMap[modelName].basePosePortName, modelConfigurationMap[modelName].basePosePort.getName()))
            {
            ...
            if ( !modelConfigurationMap[modelName].jointList.empty() &&
                 !yarp.isConnected(modelConfigurationMap[modelName].jointPositionPortName, modelConfigurationMap[modelName].jointPositionPort.getName()))
            {
            ...
            if ( modelConfigurationMap[modelName].visualizeWrenches &&
                 !yarp.isConnected(modelConfigurationMap[modelName].wrenchPortName, modelConfigurationMap[modelName].wrenchPort.getName()))
            {
                while (!yarp.connect(modelConfigurationMap[modelName].wrenchPortName, modelConfigurationMap[modelName].wrenchPort.getName()))
                {
                    yInfo() << LogPrefix << "failed to connect to the port " << modelConfigurationMap[modelName].wrenchPortName;
            ...
            if (modelConfigurationMap[modelName].visualizeWrenches)
            {
                ...
                {   
                    for (size_t modelVectorIndex = 0; modelVectorIndex < modelConfigurationMap[modelName].wrenchSourceLinks.size(); modelVectorIndex++)
                    {
                        linkTransform = viz.modelViz(modelName).getWorldLinkTransform( modelConfigurationMap[modelName].wrenchSourceLinkIndices.at(modelVectorIndex));
                        for (size_t i = 0; i < 3; i++)
                        {
                            force.setVal(i,  modelConfigurationMap[modelName].forceScalingFactor * wrenchValuesVector->data()[6 * modelVectorIndex + i]);
                        }
