...
WBTConfigRobot.UrdfFile  = 'iCubGenova09/model.urdf';
...
ConfigRobot.Gains.GainPostural.Kd = 0 * 2 * ((ConfigRobot.Gains.GainPostural.Kp).^(1/2)) / 10;
...
ConfigRobot.Gains.GainMomentum.Kd = 0 * 2 * ((ConfigRobot.Gains.GainMomentum.Kp).^(1/2)) / 10;
...
ConfigRobot.StateMachine.jointPos = [zeros(1,ConfigRobot.ROBOT_DOF);
                                     jointPos('Height35_bottom');
                                     jointPos('Height75_bottom')];
...
ConfigRobot.StateMachine.controlledLinksKp = [0; 0; 100]*30;
ConfigRobot.StateMachine.DeltaPhiMax = [0.05; 0.05; 0.05]*10;
